...
void ofxOpticalFlowFarneback::draw(int width, int height,  float lineScale, int res, float threshold) {
    bool rightSize = (sizeSml.width == width && sizeSml.height == height);
    ...
    for(int x=0; x<width; x+=res) {
        for(int y=0; y<height; y+=res) {
            ...
            if(vel.length() < threshold) {  // smaller than threshold, no point drawing.
                continue;
            }
...
void ofxOpticalFlowFarneback::drawColored(int width, int height,  float lineScale, int res, float threshold) {
    bool rightSize = (sizeSml.width == width && sizeSml.height == height);
